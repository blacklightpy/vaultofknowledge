...
var html_url_prefix = "/vaultofknowledge";
var CONFIGURED_HTML_URL_PREFIX = "/vaultofknowledge";
...
var gzip_hash = '300623953868842025819800347022702377851'                       // used to check whether the localStorage data is stale
...
function set_rel_paths(){
    ...
    for (let el of list) {
        if (el.dataset.external === "true") continue;
        let href = el.getAttribute('href');
        if (href){
...
function get_html_url_prefix(){
    return '/vaultofknowledge'  // this value is replaced by the actual url prefix when compiled
}
